...
def create_model(args, Y_train, learning_rate = 5e-5, lm = "bert-base-uncased",
                 batchsize = 16, num_epochs = 2, task_type = "A"):
    """
...
def train_model(model, tokens_train, Y_train,
    ...
    chkpt_calllback = tf.keras.callbacks.ModelCheckpoint(f"{best_modelname}_task_{task_type}",
    ...
    test_set_predict(model, tokens_dev, Y_dev, "bestmodel-on-dev",
                     encoder, False, task_type= task_type)
    return model
...
def main():
    ...
    model, tokenizer = create_model(args, Y_train, learning_rate = args.learning_rate, 
                                    ...
                                    num_epochs = args.num_epochs, task_type= args.task_type)
